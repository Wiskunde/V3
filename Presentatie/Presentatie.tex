...
\begin{columns}
        ...
        \only<3>{\input{afbeeldingen/fig_reguliere_betegeling_met_driehoeken}}
				\only<4>{\input{afbeeldingen/fig_reguliere_betegeling_met_vierkanten}}
				\only<5>{\input{afbeeldingen/fig_reguliere_betegeling_met_zeshoeken}}
\end{column}
...
\begin{frame}
  \frametitle{Les 3: Vergelijkingen op lossen met oppervlakten}
  \framesubtitle{Didactische wenken {\scriptsize(vervolg)}}
  \begin{enumerate}
    \item Onderwijsleergesprek
      \begin{itemize}
        \item Vierkanten tekenen met een lengte $x$ die onbekend is
